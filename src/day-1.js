...
For example, if you pair up a 3 from the left list with a 7 from the right list, the distance apart is 4; if you pair up a 9 with a 3,
...
export function executeDay1() {
  ...
  for (let i = 0; i < inputOne.length; i++) {
    // diff part searching
    const elOne = inputOne[i];
    const elTwo = inputTwo[i];
    const iterDiff = calculateDifference(elOne, elTwo);
    diff.push(iterDiff);

    // number of occurrence part searching
    const sim = calculateOccurrence(elOne, inputTwo);
    similarity.push(sim * elOne);
  }


  console.log(`Diff array length: "${diff.length}"`);
  console.log(`Similarity array length: "${similarity.length}"`);

  const diffSum = diff.reduce((acc, curr) => acc + curr, 0);
  console.log(`Sum for the difference: "${diffSum}"`);

  const simSum = similarity.reduce((acc, curr) => acc + curr, 0);
  console.log(`Sum for the similarity: "${simSum}"`);

}

function calculateDifference(a, b) {
  const isFirstGreater = a > b;
  const iterDiff = isFirstGreater ? a - b : b - a;

  return iterDiff;
}


function calculateOccurrence(el, secondArr) {
  let count = 0;
