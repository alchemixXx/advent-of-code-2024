...
export function executeDay5() {
  console.log('Executing day 5');
  ...
  57|25
  57|42
  42|77`.split('\n').map((el => el.split('|').flat().map(Number))).reduce((agg, el, index) => {
    agg[index] = el
    return agg
  }, {});
  // 97|13
  ...
  19,44,72,23,94,14,74,62,42,85,27,61,29,28,11,89,75,57,25,81,69,53,98`.split('\n').map(el => el.split(',').flat().map(Number));
  console.log(`Raw lines count: ${lines.length}`)

  const { correctRows, incorrectRows } = getProcessedRows(rules, lines);

  console.log(`Correct rows count: ${correctRows.length}`)
  console.log(`Incorrect rows count: ${incorrectRows.length}`)

  const sum = calculateSum(correctRows);
  console.log(`Sum for correct rows: "${sum}"`)

  console.log('Fixing incorrect rows')
  fixIncorrectLines(rules, incorrectRows)

  const sumFixed = calculateSum(incorrectRows);
  console.log(`Sum for fixed rows: "${sumFixed}"`)
}

function getProcessedRows(rules, lines) {
  const correctRows = [];
  const incorrectRows = [];

  for (const line of lines) {
    if (isCorrectLine(rules, line)) {
...
function isCorrectLine(rules, line) {
  const relatedRules = getRelatedRules(rules, line)

  for (const rule of relatedRules) {
    if (isLineFitsRule(rule, line)) {
      continue;
    }
...
function getRelatedRules(rules, line) {
  let relatedRulesSet = new Set();
  for (const el of line) {
    for (const [key, values] of Object.entries(rules)) {
      if (values.includes(el)) {
        relatedRulesSet.add(key)
      }
    }
  }
  ...
  for (const el of relatedRulesSet) {
    const rule = rules[el];
...
function isLineFitsRule(rule, line) {
  const indexFirst = line.findIndex(el => el === rule[0]);
  const indexSecond = line.findIndex(el => el === rule[1]);
...
function fixIncorrectLines(rules, lines) {
  for (const line of lines) {
    fixIncorrectLine(rules, line)
  }
}

function fixIncorrectLine(rules, line) {
  const relatedRules = getRelatedRules(rules, line)
  ...
  for (const rule of relatedRules) {
    ...
    reorderByRule(rule, line);
  }

  if (!isCorrectLine(rules, line)) {
    return fixIncorrectLine(rules, line)
  }


}

function reorderByRule(rule, line) {
  const indexFirst = line.findIndex(el => el === rule[0]);
  const indexSecond = line.findIndex(el => el === rule[1]);

  const elFirst = line[indexFirst];
  const elSecond = line[indexSecond];
  line[indexFirst] = elSecond;
  line[indexSecond] = elFirst;
...
function calculateSum(rows) {
  ...
  for (const row of rows) {
    const middleElementIndex = Math.floor(row.length / 2)
    sum += row[middleElementIndex]
  }
