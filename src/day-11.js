...
export function executeDay11() {
  console.log('Day 11: Plutonian Pebbles');
  let input = '4022724 951333 0 21633 5857 97 702 6'.split(' ').map(Number);
  // input = '125 17'.split(' ').map(Number);

  const resultPart1 = blinkStonesPart1(input, 25);
  console.log('Result Part1:', resultPart1.length);

  const resultPart2 = blinkStonesPart2(input, 75);
  console.log('Result Part2:', resultPart2);
}


function blinkStonesPart1(input, times) {
  let stones = JSON.parse(JSON.stringify(input));
...
function blinkStonesPart2(input, times) {
  let totalLength = 0;
  let map = new Map();

  for (const el of input) {
    const elementParts = blinkPart2(el, times, map);
    totalLength += elementParts;
  }
...
function blinkStones(stones) {
  ...
  for (let i = 0; i < stones.length; i++) {
    let stone = stones[i];
    const { firstElement, secondElement } = blinkStone(stone);

    if (firstElement !== undefined) {
      newStones.push(firstElement);
    }

    if (secondElement !== undefined) {
      newStones.push(secondElement);
    }
  }
...
function blinkStone(stone) {
  ...
  if (isEven(stone)) {
    ...
    return { firstElement: Number(str.substring(0, half)), secondElement: Number(str.substring(half)) };
  }


  return { firstElement: stone * 2024 };
}

function isEven(stone) {
  return String(stone).length % 2 === 0;
}

function blinkPart2(stone, times, map) {
  if (times === 0) {
    return 1;
  };
  ...
  const { firstElement, secondElement } = blinkStone(stone);
  if (firstElement !== undefined) {
    res += blinkPart2(firstElement, times - 1, map);
  }
  if (secondElement !== undefined) {
    res += blinkPart2(secondElement, times - 1, map);
  }
